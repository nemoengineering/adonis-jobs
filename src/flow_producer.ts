...
export class FlowProducer<KnownWorkers extends Record<string, WorkerManagerWorkerFactory>> {
  ...
  async add<Flow extends FlowJobArg<KnownWorkers>>(flow: Flow) {
    return await this.#producer.add(flow as BullFlowJob)
  }

  async addBulk<Flow extends FlowJobArg<KnownWorkers>>(flow: Flow[]) {
