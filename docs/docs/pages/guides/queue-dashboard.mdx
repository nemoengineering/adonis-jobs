...
To enable job deletion and re-running directly from the dashboard, make sure to exclude the dashboard routes from [CSRF protection](https://docs.adonisjs.com/guides/security/securing-ssr-applications#config-reference) in your `config/shield.ts` file. Exempt all routes starting with `/admin/queue` under the csrf option:
