...
### Custom or External Telemetry

By default, the package uses `bullmq-otel` to instrument all Queue, Worker, and FlowProducer instances. If you are using an external instrumentation library that patches BullMQ, you may end up with **duplicate spans**.

To avoid this, you can disable the built-in telemetry:
...
You can also provide your own implementation of BullMQ's `Telemetry` interface:

```typescript
import { MyCustomTelemetry } from './my_custom_telemetry'
...
When the `telemetry` option is not set, the default `bullmq-otel` behavior is preserved.
